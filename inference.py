...
def GCNfold_prediction(rna_seq, GCNfold, one_type=True, threshold=0.35):
    edge_feature = bpps(rna_seq, "vienna_2")
    node_feature = extract_rna_node_features(rna_seq)
    mask = possible_pairs(rna_seq)
    out = GCNfold.predict([node_feature[np.newaxis,...], edge_feature[np.newaxis,...], mask[np.newaxis, :, :, np.newaxis]])
...
GCNfold = keras.models.load_model(model_path, custom_objects={"GCSConv":spektral.layers.GCSConv, 
...
predict_structure = GCNfold_prediction(rna_seq, GCNfold, threshold=0.35)
